...
export default function DashboardPage() {
    ...
    const fetchData = async () => {
        try {
            const [chart, summaryRes, recent, porfileRes] = await Promise.all([
                    fetchMonthlyChart(),
                    fetchMonthlySummary(),
                    fetchTransaction(),
            ...
            setUser(porfileRes.data || {});
        } catch (error) {
            if (error instanceof Error) {
                console.log({message: error.message, type:"danger"});
            } else {
                console.log({message: "Terjadi Kesalahan", type:"danger"});
            } 
        }
    }
    ...
    <div className="p-3 space-y-6">
      ...
      <div className="flex flex-col gap-8 text-white bg-gradient-to-r
       from-indigo-900 to-indigo-600 rounded-xl p-6 shadow-2xl">
        {/* top content */}
        <div className="flex justify-between items-start flex-wrap gap-2">
            <div>
                <h2 className="text-3xl font-semibold">Wellcome Back, {user.name}</h2>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="bg-white p-6 pb-16 rounded-lg shadow-2xl lg:col-span-3 h-[61vh]">
                <div className="flex justify-between  items-center mb-4">
                    <h3 className="text-lg font-bold">Mony Flow</h3>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-2xl overflow-auto h-full">
                <div className="flex justify-between items-center mb-3">
                    <h3 className="text-mb font-bold">Recent Transaction</h3>
                </div>
                <table className="w-full text-xs text-left">
                    ...
                    <tbody>
                        {recentTransaction.map((tx, i:number) => (
                        ...
                        {recentTransaction.length === 0 && (
                            <tr>
                                <td colSpan={2} className="text-center py-4 text-gray-400">No Recent Transaction</td>
                            </tr>
                        )}
                    </tbody>
                </table>
